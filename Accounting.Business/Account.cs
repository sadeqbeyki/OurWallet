    ...
    public class Account
    {
        public static ReportViewModel ReportFromMain()
        {
            ...
            using (UnitOfWork db = new UnitOfWork())
            {
                ...
                var recive = db.AccuntingRepository.Get(a => a.TypeID == 1 && a.DateTime >= startDate && a.DateTime <= endDate).Select(a => a.Amount).ToList();
                var pay = db.AccuntingRepository.Get(a => a.TypeID == 2 && a.DateTime >= startDate && a.DateTime <= endDate).Select(a => a.Amount).ToList();
